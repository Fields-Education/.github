...
The gate is evaluated against live PR state when the job starts:

- Draft pull requests are skipped automatically.
- Non-draft pull requests run Warden only when the `warden:on` label is present
  (case-insensitive). Applying labels directly requires triage permission or
  higher.
- The shared `reviewer:off` label overrides `warden:on` and disables Warden
  along with the org's opencode-based review system.

The plain `warden` label is reserved for issues and pull requests generated by
Warden Sweep, so the review gate uses the distinct `warden:on` label.

Authorized `/warden` and `/reviewer` comment commands can change these labels
through `./.github/workflows/review-commands.yml`. The Warden gate itself does
not scan comments; its decision is based on label state.
...
A gated-off run still completes successfully, so the required-workflow ruleset
is satisfied without analysis.
...
After changing `warden:on` or `reviewer:off`, re-run the Warden check from the
Checks UI (or push a new commit) for it to take effect. If an analysis is
already running on a PR that is too large to finish, cancel the run first,
then re-run it after changing the labels.
...
types make label changes take effect immediately. Adding `warden:on` starts
analysis; removing it starts a gated-off run, and the concurrency group cancels
any analysis already in flight.
...
1. adds or removes the matching review-control labels: `/warden run` adds
   `warden:on`, `/warden skip` removes it, and `/reviewer` commands also update
   `reviewer:off`; missing labels are created before they are added
2. reacts to the comment with an eyes emoji as acknowledgement
3. re-syncs each affected review workflow: when `warden:on` or `reviewer:off`
...
Repositories without the stub still honor direct label changes, but comment
commands are not processed. In ruleset-enforced repositories, a direct label
change takes effect on the next push or a manual re-run from the Checks UI.
